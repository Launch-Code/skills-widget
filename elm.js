...
Elm.Selectable.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      var className = A2($Basics._op["++"],"selectable-",model.isSelected ? "on" : "off");
      var onClickAddress = A2($Signal.forwardTo,address,function (_p1) {    return Toggle;});
      var checkbox = A2($Html.input,_U.list([$Html$Attributes.type$("checkbox"),$Html$Attributes.checked(model.isSelected)]),_U.list([]));
      return A2($Html.button,
      _U.list([A2($Html$Events.onClick,onClickAddress,NoOp),$Html$Attributes.$class(className)]),
      _U.list([checkbox,$Html.text(model.name)]));
   });
...
Elm.MultiSelect.make = function (_elm) {
   ...
   var view = F2(function (address,model) {    return A2($Html.div,_U.list([$Html$Attributes.$class("multiselect")]),A2($List.map,itemView(address),model));});
...
Elm.SkillsWidget.make = function (_elm) {
   ...
   var multiSelectView = F3(function (msAddress,msModel,msName) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("multiselect-container")]),
      _U.list([A2($Html.h3,_U.list([]),_U.list([$Html.text(msName)])),A2($MultiSelect.view,msAddress,msModel)]));
   });
   var update = F2(function (action,model) {
      var _p2 = action;
      switch (_p2.ctor)
